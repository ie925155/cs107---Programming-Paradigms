...
static void CFGGeneration(Definition& definition, map<string, Definition>& grammar)
{
    Production production = definition.getRandomProduction();
    vector<string>::const_iterator curr = production.begin();
    while(curr != production.end()){
      if(grammar.find(*curr) == grammar.end())
        cout <<  *curr << " ";
      else
        CFGGeneration(grammar[*curr], grammar);
      curr++;
    }
}

static void randomSentenseGeneration(map<string, Definition>& grammar)
{
    Definition startDef = grammar["<start>"];
    Production production = startDef.getRandomProduction();
    vector<string>::const_iterator curr = production.begin();
    while(curr != production.end()){
      if(grammar.find(*curr) == grammar.end()) //the token is terminal
        cout << *curr << " ";
      else //nonterminal
          CFGGeneration(grammar[*curr], grammar);
      curr++;
...
int main(int argc, char *argv[])
{
  ...
  randomSentenseGeneration(grammar);
